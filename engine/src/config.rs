...
//
// resolution: CLI > env > .memvis (cwd upward) > ~/.config/memvis/config > glob auto-detect
...
fn global_config_path() -> PathBuf {
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        PathBuf::from(xdg).join("memvis/config")
    } else if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".config/memvis/config")
    } else {
        PathBuf::from("/etc/memvis/config")
    }
}

pub fn global_config_dir() -> PathBuf {
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        PathBuf::from(xdg).join("memvis")
    } else if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".config/memvis")
    } else {
        PathBuf::from("/etc/memvis")
    }
}
...
pub fn generate_global_config(dr_home: Option<&str>, tracer: Option<&str>) -> String {
    ...
    out.push_str("# memvis global configuration\n");
    out.push_str("# Generated by 'memvis setup'. Edit freely.\n");
    out.push_str("#\n");
    out.push_str("# Resolution order: CLI flag > env var > .memvis (project) > this file > auto-detect\n");
    out.push_str("#\n");
    out.push_str("# Paths\n");
    out.push_str("# -----\n");
    ...
    out.push_str("\n");
    out.push_str("# Explicit path to libmemvis_tracer.so. Auto-detected relative to memvis binary if unset.\n");
    if let Some(t) = tracer {
        out.push_str(&format!("paths.tracer = {}\n", t));
    } else {
        out.push_str("# paths.tracer = /home/you/memvis/build/libmemvis_tracer.so\n");
    }
    out.push_str("\n");
    out.push_str("# Include another config file (single level, no recursion).\n");
    out.push_str("# Useful for Docker/CI: ship /etc/memvis/site.conf with paths baked in.\n");
    out.push_str("# include = /etc/memvis/site.conf\n");
    out.push_str("\n");
...
pub fn generate_project_config(target_name: &str, tripwire: Option<&str>) -> String {
    let mut out = String::new();
    out.push_str("# memvis project configuration\n");
    out.push_str("# Generated by 'memvis init'. Edit freely.\n");
    out.push_str("#\n");
    out.push_str("# Target profiles: define per-binary defaults.\n");
    out.push_str("# When you run 'memvis <target>', these settings are applied automatically.\n");
