...
memvis setup                                       # one-time: locates DynamoRIO, writes config
cmake -B build -DDynamoRIO_DIR=$DYNAMORIO_HOME/cmake
cmake --build build -j$(nproc)
cargo build --release --manifest-path engine/Cargo.toml

# 2. Create a project profile (one-time per target)
memvis init /path/to/my_server                     # auto-detects tripwire, enables topology + heatmap

# 3. Run — that's it
memvis run /path/to/my_server
```
...
memvis run ./heap_chain                            # headless snapshot, auto-exits on idle
```

### Project Profiles (`.memvis`)

`memvis init` creates a `.memvis` file in the current directory with
auto-detected settings. This eliminates repetitive CLI flags:

```ini
# .memvis — generated by 'memvis init'
...
```sh
memvis run /path/to/nginx                          # reads .memvis, applies all settings
memvis run /path/to/nginx -- --override-flag       # CLI args after '--' replace profile args
```
...
and `build/libmemvis_tracer.so`. The engine auto-discovers the tracer from
...
```sh
memvis run ./my_program                        # headless snapshot
memvis run ./my_program --live                 # interactive TUI
memvis run ./my_program --topology topo.jsonl  # export structural graph
...
```sh
memvis run --tripwire aeProcessEvents /path/to/redis-server --port 6399
```

The tripwire is auto-detected by `memvis init` for common servers. The engine
...
```
 ...
 │                           │  ctl per PID   │                                   │
 │  tracer.c                 │  tripwire_hit  │  memvis       main.rs  (live)     │
 │  memvis_bridge.h          │   (atomic)     │  memvis-lint  lint.rs  (static)   │
 │                           │                │  memvis-diff  diff.rs  (offline)  │
 │  phase-delayed JIT:       │                │  memvis-check check.rs (CI/CD)    │
 │  BOOT → tripwire → TRACE  │                │                                   │
 │                           │                │  config.rs     (.memvis profiles) │
...
| Symptom | Cause | Fix |
|---|---|---|
| `cannot find drrun` | DynamoRIO not found | Run `memvis setup` or `export DYNAMORIO_HOME=/path/to/DynamoRIO-Linux-*` |
...
| Premature exit on server | Tripwire not set or not found | Use `memvis init <target>` to auto-detect, or `--tripwire <symbol>` |
